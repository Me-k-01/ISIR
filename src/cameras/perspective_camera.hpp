...
namespace RT_ISICG
{
	class PerspectiveCamera : public BaseCamera
	{
	  public:
		...
		{
			// VP = vTL + p_sx * vU - p_sy * vV
			//rd = vp - c / ||vp-c||
			Vec3f posPoint = _viewportTopLeftCorner + _viewportU * p_sx + -_viewportV * p_sy; // Position du point du plan dans l'espace monde
			Vec3f direction = posPoint - _position; // vecteur(A, B) = B-A

			return Ray( _position, glm::normalize(direction)); // Ray( Vec3f( 0.f ), Vec3f( 0.f, 0.f, 1.f ) );
		}
